...
5. **Verify — free only.** For each flow, run `md flows/<name>.md --_dry-run` and show the user the command plan plus which rung of the ladder chose the engine. Call out any inline commands that were deliberately skipped. If a dry run fails, fix the flow and re-verify. NEVER do a real run.
